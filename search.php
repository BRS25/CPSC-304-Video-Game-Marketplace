...
function printResult2($result) { //prints results from a select statement
	...
	<th>price</th><th>Contact</th></tr>";

	while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {

		//Display results.
		echo "<tr><td>" . $row[2] . "</td><td>" . $row[0] . "</td><td>" . $row[1]
		 . "</td><td> <a href = 'mailto:"  . $row[3]
     		. "'> E-Mail</a> </td></tr> <br>"; //or just use "echo $row[0]"
		//echo '<a href = "hello.php"> Link</a>';
...
{	 
	if (array_key_exists('search', $_POST)) 
	{
		echo "<br>Here are the listings matching your search:<br>";	
			...
			if ($price != "")
			{
				...
				{
					print("<script>window.alert('ALERT: Price must be a numerical value for it to be considered.');</script>");
				}
			}
			if ($MSRP != "")
			{
				...
				{
					print("<script>window.alert('ALERT: MSRP must be a numerical value for it to be considered.');</script>");
				}
			}
			if ($location != "")
			{
			...
			{
					print("<script>window.alert('ALERT: You must specify to show the least/most expensive result in order for company averaging to be considered.');</script>");
			}
			...
			//{
				if ($minimal == true)
				{
					if ($sort == 'lp.username' || $sort == 'lp.price' || $sort == 'lp.title')
					{
					...
					{
						print("<script>window.alert('ALERT: You may only sort on title, username, or price when enabling the minimal view.');</script>");
					}
	...
	else if (array_key_exists('init', $_POST)) 
	{
		...
		// executePlainSQL("CREATE TABLE account (username CHAR(30),.php
		// 	Password CHAR(30),
		// 	Recovery CHAR(30),
		// 	member_since DATE,
		// 	phone CHAR(30),
